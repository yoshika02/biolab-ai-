...
export async function POST(request: NextRequest) {
    try {
        ...
        const steps = (protocol.steps || []).map((s, i) => `${i + 1}. ${s.instruction}${s.duration_min ? ` (${s.duration_min} min)` : ""}${s.notes ? ` — ${s.notes}` : ""}`).join("\n");
        const reagents = (protocol.reagents || []).map((r) => `- ${r.name}${r.quantity ? ` ${r.quantity}${r.unit || ""}` : ""}${r.supplier ? ` (${r.supplier})` : ""}`).join("\n");
...
Provide a thorough, structured, and practical answer. Use headings, bullet points, and numbered lists where appropriate. Be specific to this protocol.`;
